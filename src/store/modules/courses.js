...
async function parseRequirement(courseCode) {
    ...
    if (!hasNumber.test(courseCode)){
        ...
        if (courseCode === "NON-MATH") {
            ...
            const response = await axios.get("http://127.0.0.1:8000/api/course-info/filter", {
                params: {
                    start: 0,
                    end: 499,
                    code: courseCode,
        ...
        if (courseCode[courseCode.length - 1] === "-") {
            ...
            if(split[1] === "LAB"){
                response = await axios.get("http://127.0.0.1:8000/api/course-info/filter", {
                    params: {
            ...
            const response = await axios.get("http://127.0.0.1:8000/api/course-info/get", {
                params: {
                    pk: courseCode,
                }
            }).catch(error => {
